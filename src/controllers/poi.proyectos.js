...
const verProyectos = async (req, res) => {
  try {
    const resultado = await db.query('SELECT * FROM sis_catastro_verificacion.tbl_proyectos ORDER BY codigo');
...
const verProyecto = async (req, res) => {
  try {
    ...
    const resultado = await db.query('SELECT * FROM sis_catastro_verificacion.tbl_proyectos WHERE codigo = $1', [codigo]);
